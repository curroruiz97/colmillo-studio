...
/** Deepest dent, as a share of the tile's short side, clamped in pixels. */
...
/** Half the width of the dent, as a share of the tile's short side. */
 ...
 * Fine pointers with motion allowed only: touch, keyboard and reduced motion
 * keep the still image and the CSS title reveal.
 */
...
function bindTile(link: HTMLElement): () => void {
  ...
  if (!surface || !frame) return () => undefined;
  ...
  // Layout size, not the transformed box: the section stack may scale the
  // rail while it is covered, and the path is drawn in the surface's own units.
  const measure = () => {
